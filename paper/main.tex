...
\texttt{judgment-only baseline} & none & \texttt{J1 only} & sanity check for first-pass stability \\
\bottomrule
\end{tabular}
...
The completed confirmatory evidence base is stored in two selected-v2 analysis files:
\begin{itemize}
  ...
  \item plus a 40-item \texttt{judgment-only baseline} sanity subset for each model.
\end{itemize}

Decoding was deterministic with temperature 0.0, fixed seed 42, \texttt{max\_judgment\_tokens}=80, and \texttt{max\_explanation\_tokens}=100. The completed runs use the selected frame variants only. The active 7B selected-v2 file is a metadata-normalized export of the locked raw 7B run, preserving decoded outputs while backfilling the current schema fields; Appendix~G documents this explicitly. The paraphrase-family audit infrastructure exists in code, but no full family-audit rows are included in the current confirmatory results and none are promoted into the paper's substantive claims.
...
\begin{tabular}{@{}p{0.22\linewidth} p{0.16\linewidth} >{\centering\arraybackslash}p{0.06\linewidth} >{\centering\arraybackslash}p{0.06\linewidth} p{0.16\linewidth} >{\centering\arraybackslash}p{0.06\linewidth} >{\centering\arraybackslash}p{0.06\linewidth} p{0.16\linewidth}@{}}
\toprule
\shortstack[l]{Contrast} & \shortstack[l]{7B est.\\{}[95\% CI]} & \shortstack[c]{7B\\$p$} & \shortstack[c]{7B\\$d_z$} & \shortstack[l]{0.5B est.\\{}[95\% CI]} & \shortstack[c]{0.5B\\$p$} & \shortstack[c]{0.5B\\$d_z$} & \shortstack[l]{Reading} \\
\midrule
C-pre $-$ S-pre on \texttt{J1} heart shift & +1.67 pp [+0.00, +4.17] & 0.494 & 0.13 & -0.83 pp [-2.50, +0.00] & 1.000 & -0.09 & Modest 7B heart increment; none at 0.5B. \\
C-pre $-$ S-pre on \texttt{J1} act shift & -0.83 pp [-2.50, +0.00] & 1.000 & -0.09 & +0.00 pp [+0.00, +0.00] & 1.000 & 0.00 & Act-level movement stays near zero. \\
C-post $-$ S-post on raw explanation score & -1.67 pp [-15.83, +11.67] & 0.908 & -0.02 & -4.17 pp [-20.83, +10.00] & 0.654 & -0.05 & Raw explanation contrast is not Christian-positive. \\
C-post $-$ S-post on controlled semantic score & -5.00 pp [-13.33, +1.67] & 0.295 & -0.12 & +2.50 pp [-0.83, +6.67] & 0.372 & 0.12 & Controlled residual remains weak and unstable. \\
C-post $-$ S-post on \texttt{J1}$\rightarrow$\texttt{J2} act revision & -2.50 pp [-5.83, +0.00] & 0.239 & -0.16 & +0.00 pp [+0.00, +0.00] & 1.000 & 0.00 & Act revision stays rare, with no Christian-specific pressure. \\
C-post $-$ S-post on \texttt{J1}$\rightarrow$\texttt{J2} heart revision & -1.67 pp [-4.17, +0.00] & 0.503 & -0.13 & +0.00 pp [+0.00, +0.00] & 1.000 & 0.00 & Heart revision stays rare, with no Christian-specific pressure. \\
\bottomrule
\end{tabular}
...
The second main result is that explanation language is more prompt-sensitive than first-pass exposed judgment relative to baseline. This pattern appears in both models, but the completed 7B run is the clearest case and the one that most cleanly shows explanation movement without a stable Christian-specific residual.
...
\begin{itemize}
  ...
  \item \textbf{\texttt{judgment-only baseline}} (internal key \texttt{baseline\_judgment\_only}): system prompt $\rightarrow$ \texttt{J1} only. No frame text, no explanation stage, no \texttt{J2}.
\end{itemize}
...
\footnotesize
\begin{longtable}{@{}p{0.13\linewidth} p{0.25\linewidth} p{0.17\linewidth} p{0.07\linewidth} p{0.07\linewidth} p{0.19\linewidth}@{}}
...
\begin{verbatim}
model, split, run_id, condition, item_id, seed, temperature,
max_judgment_tokens, max_explanation_tokens, frame_mode, frame_family,
frame_variant_id, frame_position, frames_config_version,
j1_act, j1_heart, e_focus, e_text, j2_act, j2_heart, raw_trace
\end{verbatim}
...
\subsection{Figure 5}
Data slice: the two main matched-control contrasts, pre-heart and post controlled explanation. Infer: attenuation across scale within Qwen. Do not infer: a stable Christian-specific effect across model size.
