...
\caption{Overview of the project question, staged design, model comparison, and calibrated claim boundary. The figure is a study map rather than an inferential result: it summarizes the benchmark setup, the \texttt{J1 $\rightarrow$ E $\rightarrow$ J2} protocol, and the paper's central interpretive point that prompt sensitivity appears more clearly in post-hoc explanation than in first-pass exposed judgment.}
\label{fig:overview}
\end{figure}
...
\caption{First-pass shift rates by condition and model size. Heart-level pre-framing movement is visibly larger than act-level movement, while the smaller Qwen model attenuates the pattern. The figure supports a stage-dissociation argument, not a claim of strong Christian superiority.}
...
\caption{Revision rates from \texttt{J1} to \texttt{J2}. Revision is rare in both models and is used here as a secondary mechanism check. The figure matters because explanation can move while both first-pass and re-judged exposed choices remain largely stable.}
\label{fig:revision}
\end{figure}
...
\caption{Same-family scale comparison of the two most diagnostic matched-control contrasts. The modest 7B pre-heart increment attenuates in the smaller Qwen model, and the controlled explanation contrast does not form a stable Christian-specific pattern at either size.}
...
\caption{Exploratory heterogeneity by item type. This figure is used to probe where motive-sensitive effects may concentrate, not to claim confirmed category-level effects. The scale comparison again looks more like attenuation and instability than robustness success.}
...
\begin{itemize}
  \item \texttt{outputs/runs/qwen2.5\_7b\_instruct\_eval\_locked\_v1.jsonl}
