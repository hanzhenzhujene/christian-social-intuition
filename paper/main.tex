...
\caption{Data-informed overview of the completed study. The schematic summarizes the actual benchmark sizes (150-item candidate pool, 30 locked development items, 120 locked evaluation items, and a 40-item judgment-only sanity subset), the \texttt{J1 $\rightarrow$ E $\rightarrow$ J2} protocol, the matched Christian-versus-secular framing design, and the calibrated evidence pattern from the completed Qwen runs. It is an overview figure, not a standalone inferential result.}
\label{fig:overview}
\end{figure}
